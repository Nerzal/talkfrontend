...
- **Click animations (fragments)**: bullets in a `content`/`mixed` slide can be marked with `-> ` instead of `- ` to reveal them one at a time via click/arrow key, instead of showing the whole list at once

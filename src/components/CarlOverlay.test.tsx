...
describe('CarlOverlay', () => {
  ...
  it('renders nothing when presentation has Carl disabled', () => {
    const { container } = render(
      <CarlOverlay presentationEnabled={false} currentSlideAllowsCarl={true} />,
    )
  ...
  it('renders nothing when the current slide does not allow Carl', () => {
    const { container } = render(
      <CarlOverlay presentationEnabled={true} currentSlideAllowsCarl={false} />,
    )
    expect(container.firstChild).toBeNull()
  })
  ...
  it('slides in from a random side after a random delay, then slides back out', () => {
    ...
    render(<CarlOverlay presentationEnabled={true} currentSlideAllowsCarl={true} />)
  ...
  it('stops scheduling once disabled', () => {
    ...
    const { rerender } = render(
      <CarlOverlay presentationEnabled={true} currentSlideAllowsCarl={true} />,
    )
    act(() => {
      vi.advanceTimersByTime(MIN_INTERVAL_MS)
    })
    ...
    rerender(<CarlOverlay presentationEnabled={false} currentSlideAllowsCarl={true} />)
